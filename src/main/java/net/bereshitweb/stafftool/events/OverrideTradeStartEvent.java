package net.bereshitweb.stafftool.events;
...
public class OverrideTradeStartEvent extends MessageHandler {
    @Override
        ...
        if (Emulator.getIntUnixTimestamp() - this.client.getHabbo().getHabboStats().lastTradeTimestamp > 10) {
            ...
            if (room != null) {
                if (userId >= 0 && userId != this.client.getHabbo().getRoomUnit().getId()) {
                    ...
                    if(this.client.getHabbo().getHabboStats().cache.containsKey(StaffToolUtils.CHECKINVKEY))  {
                        this.client.getHabbo().whisper(Emulator.getTexts().getValue("checkinventory.error.trade_disabled"));
                        return;
                    }

                    if (targetUser != null) {
                        if(targetUser.getHabboStats().cache.containsKey(StaffToolUtils.CHECKINVKEY)) {
                            this.client.getHabbo().whisper(Emulator.getTexts().getValue("checkinventory.error.cant_trade"));
                            return;
                        }
